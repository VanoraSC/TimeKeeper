...
public class TimeDeletionPanel extends AbstractTaskChangerGridBagJPanel
	...
	for (TaskRow tr : TaskChooserPanel.getTasksList()) {
		...
		for (int i = 0; i < tr.getSubTasks().size(); i++) {
		    if (tr.getSubTasks().get(i).getName()
			    .equalsIgnoreCase(subTaskName)) {
			...
			Object[][] objectSubTaskTimes = new Object[timesList
				.size()][4];
			String[] columnNames = new String[] { "ID", "StartTime",
				"EndTime", "Delete" };

			for (int j = 0; j < timesList.size(); j++) {
			    objectSubTaskTimes[j][0] = timesList.get(j).getId();
			    objectSubTaskTimes[j][1] = timesList.get(j)
				    .getStartTime();
			    objectSubTaskTimes[j][2] = timesList.get(j)
				    .getEndTime();
			    objectSubTaskTimes[j][3] = "Delete";
			}

			if (subTaskTimesTable == null)
			    subTaskTimesTable = new JTable(
				    new DefaultTableModel(objectSubTaskTimes,
					    columnNames));
			else
			    subTaskTimesTable.setModel(new DefaultTableModel(
				    objectSubTaskTimes, columnNames));

			final TableColumnModel columnModel = subTaskTimesTable
				.getColumnModel();

			for (int column = 0; column < subTaskTimesTable
				.getColumnCount(); column++) {
			    int width = 0; // Min width
			    for (int row = 0; row < subTaskTimesTable
				    .getRowCount(); row++) {

				TableCellRenderer renderer = subTaskTimesTable
					.getCellRenderer(row, column);
				Component comp = subTaskTimesTable
					.prepareRenderer(renderer, row, column);
				width = Math.max(
					comp.getPreferredSize().width + 50,
					width);
		...
		DeleteSubTaskTimeAction delete = new DeleteSubTaskTimeAction(
	...
	public void actionPerformed(ActionEvent e) {
	    ...
	    TimeRow timeRow = timesList
		    .get(Integer.valueOf(e.getActionCommand()));

	    Object[] options = { "Yes", "No" };
	    int answer = JOptionPane.showOptionDialog(null,
		    "Are you sure you want to delete Time ID " + timeRow.getId()
			    + "?",
