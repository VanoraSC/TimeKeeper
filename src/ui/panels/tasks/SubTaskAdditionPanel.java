...
public class SubTaskAdditionPanel extends AbstractTaskChangerGridBagJPanel
	...
	public SubTaskAdditionPanel() {
		...
		newTaskPanel.setPreferredSize(new Dimension((int) taskComboBox.getPreferredSize().getWidth() + 300,
		...
		addSubTaskButton.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				...
				try {
					...
					while (rs.next()) {
						if (rs.getString("name").equalsIgnoreCase(newSubTaskNameTextField.getText())) {
							JOptionPane.showMessageDialog(null,
									"SubTask Already Exists for Task " + (String) taskComboBox.getSelectedItem() + "!!!");
				...
				try {
					...
					if (ps.executeUpdate() != 1) {
						...
						JOptionPane.showMessageDialog(null, "\"" + newSubTaskNameTextField.getText() + "\""
								+ " Successfully Added to Task " + (String) taskComboBox.getSelectedItem() + ".");
