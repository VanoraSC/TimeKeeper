...
public class TaskChooserPanel extends AbstractGridBagJPanel
	...
	try {
		...
		PreparedStatement ps2 = dbCon
			.prepareStatement(selectSubTasksForTask);
		ps2.setInt(1, tr.getId());
		ResultSet rs2 = ps2.executeQuery();

		while (rs2.next()) {
		    tr.AddSubTask(new SubTaskRow(rs2.getInt("sub_task_id"),
			    rs2.getInt("tasks_id"), rs2.getString("name")));
		}
		tasksList.add(tr);
	    }
	...
	synchronized (this) {
		...
		if (tr.getName().equalsIgnoreCase(
		    ...
		    if (DEBUG)
			System.out.println("Task: " + tr.getName() + " has "
				+ tr.getSubTasks().size() + " sub tasks");
