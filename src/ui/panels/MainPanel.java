...
public class MainPanel extends JPanel
	implements UIConstants, TaskChangeListener, TaskChangeNotifer,
	TimeChangeListener, ListenerConstants, InsertStatements {
	...
	this.setPreferredSize(
		new Dimension((int) this.getPreferredSize().getWidth() + 300,
	...
	synchronized (this) {
		...
		if (currentState == State.started) {
		    ...
		    StartStopTime sst = new StartStopTime(
		...
		endTime = System.currentTimeMillis();
		StartStopTime sst = new StartStopTime(new Timestamp(startTime),
	...
	if (DEBUG) {
		...
		System.out.println("Started " + sst.getStartTime()
			+ ", Stopped " + sst.getStopTime());
	    }
	...
	outer: for (TaskRow tr : TaskChooserPanel.getTasksList()) {
		...
		for (SubTaskRow str : tr.getSubTasks()) {
		    if (str.getName()
			    .equalsIgnoreCase(tcp.getCurrentSubTaskName())) {
